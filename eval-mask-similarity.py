...
def evaluate_masked_similarity(args):
    ...
    plt.title(f"Nearest Centroid Confusion Matrix ({args.class_name.upper()} - Masked)", fontsize=14, pad=15)
    plt.xlabel("Predicted Class", fontsize=12)
    plt.ylabel("True Class", fontsize=12)
    plt.tight_layout()
    cm_path = os.path.join(save_dir, "confusion_matrix_masked.png")
    ...
    if args.use_wandb:
        ...
        wandb_log_dict = {
            # Accuracy Metric
            "masked_eval/nearest_centroid_acc": float(eval_acc_centroid),
            # Static Plot Images
            ...
            "plots/masked_confusion_matrix": wandb.Image(cm_path),
            ...
            "plots/masked_interactive_confusion_matrix": wandb.plot.confusion_matrix(
                preds=eval_preds.tolist(),
                y_true=eval_labels_np.tolist(),
                class_names=classes,
                title=f"Nearest Centroid Confusion Matrix ({args.class_name.upper()} - Masked)"
            ),
            # Numeric Tables
            "masked_eval/similarity_matrix": sim_table,
            "masked_eval/centroid_similarity_matrix": centroid_sim_table,
            "masked_eval/softmax_prob_matrix": prob_table,
            "masked_eval/confusion_matrix_table": cm_table,
            "masked_eval/confusion_matrix_normalized": cm_norm_table,
            "masked_eval/distribution_summary_table": dist_summary_table,
            "masked_eval/tsne_coordinates": tsne_coords_table,
            "masked_eval/sample_similarity_results": sample_results_table,
            # Histograms for Numeric Distribution
            "masked_eval/intra_sim_distribution": wandb.Histogram(own_sims_np),
            "masked_eval/inter_sim_distribution": wandb.Histogram(other_sims_np),
            # Overall Distribution Scalars
            "masked_eval/overall_intra_sim_mean": float(np.mean(own_sims_np)) if len(own_sims_np) > 0 else 0.0,
            "masked_eval/overall_intra_sim_std": float(np.std(own_sims_np)) if len(own_sims_np) > 0 else 0.0,
            "masked_eval/overall_intra_sim_median": float(np.median(own_sims_np)) if len(own_sims_np) > 0 else 0.0,
            "masked_eval/overall_inter_sim_mean": float(np.mean(other_sims_np)) if len(other_sims_np) > 0 else 0.0,
            "masked_eval/overall_inter_sim_std": float(np.std(other_sims_np)) if len(other_sims_np) > 0 else 0.0,
            "masked_eval/overall_inter_sim_median": float(np.median(other_sims_np)) if len(other_sims_np) > 0 else 0.0
        }
        if len(defect_sims_np) > 0:
            wandb_log_dict["masked_eval/defect_to_good_sim_distribution"] = wandb.Histogram(defect_sims_np)
            wandb_log_dict["masked_eval/overall_defect_to_good_sim_mean"] = float(np.mean(defect_sims_np))
            wandb_log_dict["masked_eval/overall_defect_to_good_sim_std"] = float(np.std(defect_sims_np))
            wandb_log_dict["masked_eval/overall_defect_to_good_sim_median"] = float(np.median(defect_sims_np))
