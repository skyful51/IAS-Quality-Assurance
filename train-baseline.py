...
def train_and_evaluate(args):
    ...
    # Calculate pairwise centroid-to-centroid similarity matrix
    ...
    plt.title("Nearest Centroid Confusion Matrix (Baseline)", fontsize=14, pad=15)
    ...
    combined_data = np.concatenate([eval_embeddings_cpu, centroids_np], axis=0)
    ...
    if args.use_wandb:
        ...
        table_data = [[classes[c]] + [float(val) for val in avg_sim_matrix[c]] for c in range(num_classes)]
        ...
        centroid_table_data = [[classes[i]] + [float(val) for val in centroid_sim_matrix[i]] for i in range(num_classes)]
        centroid_sim_table = wandb.Table(data=centroid_table_data, columns=centroid_columns)

        # 3. Softmax Probability Matrix Table
        prob_table_data = [[classes[c]] + [float(val) for val in avg_prob_matrix[c]] for c in range(num_classes)]
        prob_table = wandb.Table(data=prob_table_data, columns=columns)

        # 4. Confusion Matrix Tables (Raw Counts & Normalized Recall)
        cm_table_data = [[classes[i]] + [int(val) for val in cm[i]] for i in range(num_classes)]
        cm_table = wandb.Table(data=cm_table_data, columns=columns)

        cm_norm_data = [[classes[i]] + [float(val) for val in cm_norm[i]] for i in range(num_classes)]
        cm_norm_table = wandb.Table(data=cm_norm_data, columns=columns)
        ...
        wandb_log_dict = {
            ...
            "plots/interactive_confusion_matrix": wandb.plot.confusion_matrix(
                preds=eval_preds.tolist(),
                y_true=eval_labels_np.tolist(),
                class_names=classes,
                title="Nearest Centroid Confusion Matrix"
            ),
            # Numeric Tables
            "final_eval/similarity_matrix": sim_table,
            "final_eval/centroid_similarity_matrix": centroid_sim_table,
            "final_eval/softmax_prob_matrix": prob_table,
            "final_eval/confusion_matrix_table": cm_table,
            "final_eval/confusion_matrix_normalized": cm_norm_table,
            "final_eval/distribution_summary_table": dist_summary_table,
            "final_eval/tsne_coordinates": tsne_coords_table,
            "final_eval/sample_similarity_results": sample_results_table,
            # Histograms for Numeric Distribution
            "final_eval/intra_sim_distribution": wandb.Histogram(own_sims_np),
            "final_eval/inter_sim_distribution": wandb.Histogram(other_sims_np),
            # Overall Distribution Scalars
            "final_eval/overall_intra_sim_mean": float(np.mean(own_sims_np)) if len(own_sims_np) > 0 else 0.0,
            "final_eval/overall_intra_sim_std": float(np.std(own_sims_np)) if len(own_sims_np) > 0 else 0.0,
            "final_eval/overall_intra_sim_median": float(np.median(own_sims_np)) if len(own_sims_np) > 0 else 0.0,
            "final_eval/overall_inter_sim_mean": float(np.mean(other_sims_np)) if len(other_sims_np) > 0 else 0.0,
            "final_eval/overall_inter_sim_std": float(np.std(other_sims_np)) if len(other_sims_np) > 0 else 0.0,
            "final_eval/overall_inter_sim_median": float(np.median(other_sims_np)) if len(other_sims_np) > 0 else 0.0
        }
        if len(defect_sims_np) > 0:
            wandb_log_dict["final_eval/defect_to_good_sim_distribution"] = wandb.Histogram(defect_sims_np)
            wandb_log_dict["final_eval/overall_defect_to_good_sim_mean"] = float(np.mean(defect_sims_np))
            wandb_log_dict["final_eval/overall_defect_to_good_sim_std"] = float(np.std(defect_sims_np))
            wandb_log_dict["final_eval/overall_defect_to_good_sim_median"] = float(np.median(defect_sims_np))
