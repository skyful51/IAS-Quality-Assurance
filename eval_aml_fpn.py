...
def evaluate_aml_fpn_similarity(args):
    ...
    plt.title(f"Nearest Centroid Confusion Matrix ({args.class_name.upper()} - AML FPN)", fontsize=14, pad=15)
    plt.xlabel("Predicted Class", fontsize=12)
    plt.ylabel("True Class", fontsize=12)
    plt.tight_layout()
    cm_path = os.path.join(save_dir, "confusion_matrix_aml_fpn.png")
    ...
    if args.use_wandb:
        ...
        wandb_log_dict = {
            ...
            "plots/aml_fpn_interactive_confusion_matrix": wandb.plot.confusion_matrix(
                preds=eval_preds.tolist(),
                y_true=eval_labels_np.tolist(),
                class_names=classes,
                title=f"Nearest Centroid Confusion Matrix ({args.class_name.upper()} - AML FPN)"
            ),
            # Numeric Tables
            "aml_fpn_eval/similarity_matrix": sim_table,
            "aml_fpn_eval/centroid_similarity_matrix": centroid_sim_table,
            "aml_fpn_eval/softmax_prob_matrix": prob_table,
            "aml_fpn_eval/confusion_matrix_table": cm_table,
            "aml_fpn_eval/confusion_matrix_normalized": cm_norm_table,
            "aml_fpn_eval/distribution_summary_table": dist_summary_table,
            "aml_fpn_eval/tsne_coordinates": tsne_coords_table,
            "aml_fpn_eval/sample_similarity_results": sample_results_table,
            # Histograms for Numeric Distribution
            "aml_fpn_eval/intra_sim_distribution": wandb.Histogram(own_sims_np),
            "aml_fpn_eval/inter_sim_distribution": wandb.Histogram(other_sims_np),
            # Overall Distribution Scalars
            "aml_fpn_eval/overall_intra_sim_mean": float(np.mean(own_sims_np)) if len(own_sims_np) > 0 else 0.0,
            "aml_fpn_eval/overall_intra_sim_std": float(np.std(own_sims_np)) if len(own_sims_np) > 0 else 0.0,
            "aml_fpn_eval/overall_intra_sim_median": float(np.median(own_sims_np)) if len(own_sims_np) > 0 else 0.0,
            "aml_fpn_eval/overall_inter_sim_mean": float(np.mean(other_sims_np)) if len(other_sims_np) > 0 else 0.0,
            "aml_fpn_eval/overall_inter_sim_std": float(np.std(other_sims_np)) if len(other_sims_np) > 0 else 0.0,
            "aml_fpn_eval/overall_inter_sim_median": float(np.median(other_sims_np)) if len(other_sims_np) > 0 else 0.0
        }
        if len(defect_sims_np) > 0:
            wandb_log_dict["aml_fpn_eval/defect_to_good_sim_distribution"] = wandb.Histogram(defect_sims_np)
            wandb_log_dict["aml_fpn_eval/overall_defect_to_good_sim_mean"] = float(np.mean(defect_sims_np))
            wandb_log_dict["aml_fpn_eval/overall_defect_to_good_sim_std"] = float(np.std(defect_sims_np))
            wandb_log_dict["aml_fpn_eval/overall_defect_to_good_sim_median"] = float(np.median(defect_sims_np))
